<x-app-layout title='İşletmede Mesleki Eğitim'>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">{{ __('İşletmede Mesleki Eğitim') }} </h2>
    </x-slot>
                ...
                <form class="row d-flex justify-content-between"" method="GET" action="" enctype="multipart/form-data">
                    ...
                    @if (request()->get('search'))
                        <div class="col d-flex justify-content-end">
                            <a href="{{ route('ogrenci.imesget') }}" class="btn btn-secondary">
                                Sıfırla
                            </a>
                        </div>
                    @endif
                ...
                <tbody>
                    @foreach ($imes as $ime)
                        <tr>
                            ...
                            <td>
                                <a href="{{route('ogrenci.imeget',$ime->id)}}" class="btn btn-primary"><i class="bi bi-pencil-square"></i> İME Detayları</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
